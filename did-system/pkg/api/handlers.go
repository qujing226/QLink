...
func (s *Server) batchRegisterDID(c *gin.Context) {
	...
	// 输入验证
	if len(req.DIDs) == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "DID列表不能为空"})
		return
	}

	if len(req.DIDs) > 100 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "批量注册DID数量不能超过100个"})
		return
	}
...
func (s *Server) batchResolveDID(c *gin.Context) {
	...
	if len(req.DIDs) > 100 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "批量解析DID数量不能超过100个"})
		return
	}
	...
	for _, did := range req.DIDs {
		// 验证DID格式
		if len(did) == 0 || len(did) > 255 {
			results = append(results, gin.H{
				"did":     did,
				"success": false,
				"error":   "DID长度必须在1-255字符之间",
			})
			continue
		}
...
func (s *Server) addPeer(c *gin.Context) {
	...
	if len(req.ID) == 0 || len(req.ID) > 64 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "节点ID长度必须在1-64字符之间"})
		return
	}

	if len(req.Address) == 0 || len(req.Address) > 255 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "节点地址长度必须在1-255字符之间"})
		return
	}
	...
	if _, exists := s.peers[req.ID]; exists {
		...
		c.JSON(http.StatusConflict, gin.H{"error": "节点已存在"})
		return
	}
	s.peersMutex.RUnlock()
