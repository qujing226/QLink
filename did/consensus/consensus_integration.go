...
// NewConsensusIntegration 创建共识集成器
func NewConsensusIntegration(nodeID string, raftNode *RaftNode, didRegistry *did.DIDRegistry,
	p2pNetwork *network.P2PNetwork, config *ConsensusConfig) *ConsensusIntegration {
...
func (ci *ConsensusIntegration) ProposeDIDOperation(operation string, didDoc *did.DIDDocument) (*Proposal, error) {
	didOp := &DIDOperation{
		Operation: operation,
		DID:       didDoc.ID,
		Document:  didDoc,
