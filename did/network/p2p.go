...
func NewP2PNetwork(nodeID, address string, port int, config *NetworkConfig) *P2PNetwork {
	if config == nil {
		config = &NetworkConfig{
			MaxPeers:          50,
...
func (p2p *P2PNetwork) connectToPeer(peer *Peer) {
	...
	conn, err := net.DialTimeout("tcp", addr, p2p.config.ConnectionTimeout)
	if err != nil {
		log.Printf("连接节点失败 %s: %v", peer.ID, err)
